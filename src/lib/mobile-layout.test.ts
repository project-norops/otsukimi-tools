...
describe("mobile layout regression guards", () => {
  ...
  it("keeps calendar dates and chips readable on desktop and mobile", () => { expect(css).toMatch(/\.day-number\s*\{[\s\S]*?font-size: 14px;/); expect(css).toMatch(/@media \(max-width: 800px\)[\s\S]*?\.chip,[\s\S]*?font-size: 10\.5px;/); });
